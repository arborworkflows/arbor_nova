    ...
    <v-layout class="transform-view" row fill-height>
      <v-navigation-drawer permanent fixed style="width: 400px; min-width: 400px;">
        <v-toolbar dark flat color="primary">
          <v-toolbar-title class="white--text">Docker-PolyA-tail</v-toolbar-title>
        </v-toolbar>
        <v-container fluid>
          <v-flex xs12>
      ...
      <v-layout column justify-start fill-height style="margin-left: 400px">
        ...
        <template v-if="!running && job.status === 3">
          <v-card v-if="result.length > 0" class="mb-4 ml-4 mr-4">
            <v-card-text>PolyA Tail Results Sample</v-card-text>
  ...
  methods: {
    async run() {
      ...
      this.job = (await this.girderRest.post(
        `arbor_nova/docker_polya?${params}`,
      )).data;
	...
	// the result from the original docker container comes back as a TSV file with some junk in the 
	// beginning lines, so we will trim it up before rendering a sample of the table
	this.resultString = this.resultString.split('\n').slice(8,33).join('\n')
        //this.result = csvParse(this.resultString);
        this.result = tsvParse(this.resultString);
