  ...
  <v-app>
    <v-layout class="transform-view" row fill-height>
      <v-navigation-drawer permanent fixed style="width: 400px; min-width: 400px;">
        <v-toolbar dark flat color="primary">
          <v-toolbar-title class="white--text">Time Series Measurements</v-toolbar-title>
          <v-spacer/>
        </v-toolbar>
        <v-container fluid>
          <v-flex xs12>
            <v-select 
		label="Select the Season to Explore" 
	...
	<v-spacer></v-spacer>
      ...
      <v-layout column justify-start fill-height style="margin-left: 400px">
          <v-card class="ma-4">
            <v-card-text>
              <b>Examine time series measurements.</b> Select a season and one or more cultivars to show the time series plots 
		 for the cultivar's measurements during the season.  A separate selector allows you to pick which 
		feature you want to view. Once you have selected the season, cultivar(s), and feature, select the DISPLAY button to draw the plots.    
              <br></br>
            </v-card-text>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    async runModel() {
      ...
      const title = this.selectedSeason+': Time Series for '+this.selectedTraits
      var vegaLiteSpec = {
	  ...
	  "width" : 1000,
	    ...
	    "encoding": {
	      "x": {"field": "day_offset", "type": "quantitative"},
	      "y": { "field": this.selectedTraits, "type": "quantitative"},
	      "color": { "field": "cultivar", "type":"nominal"},
	      }
	};
